 ...
 * Updated for @stacks/transactions v7.3.1
 */
...
describe("test token streaming contract", () => {
  ...
  it("ensures contract is initialized properly and stream is created", () => {
    const latestStreamId = simnet.getDataVar("token-streaming", "latest-stream-id");
    expect(latestStreamId).toBeUint(1);

    const createdStream = simnet.getMapEntry("token-streaming", "streams", Cl.uint(0));
  ...
  it("ensures recipient can withdraw tokens over time", () => {
    ...
    // But the beforeEach adds one more block, so we're at Block 4
    // Expected to withdraw (Block 4 - Start_Block) = (4 - 0) = 4 tokens
  ...
  it("ensures sender can withdraw excess tokens", () => {
    // Block 3
    simnet.callPublicFn("token-streaming", "refuel", [Cl.uint(0), Cl.uint(5)], sender);
    ...
    // Claim tokens
    simnet.callPublicFn("token-streaming", "withdraw", [Cl.uint(0)], recipient);
  ...
  it("signature verification can be done on stream hashes", () => {
    ...
    // The hash result is a BufferCV which cvToValue converts to a hex string
    expect(hashedStream0.result).toBeDefined();

    // Verify the hash is returned as a hex string (0x prefix + 64 hex chars = 32 bytes)
    const hashValue = cvToValue(hashedStream0.result);
    expect(typeof hashValue).toBe("string");
    expect(hashValue).toMatch(/^0x[0-9a-f]{64}$/i); // 32 bytes = 64 hex chars
  });
  ...
  it("ensures timeframe and payment per block can be modified with consent of both parties", () => {
    ...
    // Just verify the hash function works
    expect(hashedStream0.result).toBeDefined();

    // Verify it returns a valid hex string hash
    const hashValue = cvToValue(hashedStream0.result);
    expect(typeof hashValue).toBe("string");
    expect(hashValue).toMatch(/^0x[0-9a-f]{64}$/i);
  });
});